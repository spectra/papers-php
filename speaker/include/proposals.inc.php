...
class Proposals {
  ...
  function create($db, $fields) {
    $rs = $db->conn->Execute("select cod,titulo,tema,idioma,publicoalvo,descricao,resumo,comentarios,ip,ip_proxy,browser,dthora,pessoa,nivel_proposta, nivel_envolvimento from propostas where cod = -1");
    $sql = $db->conn->GetInsertSQL($rs, $fields, false, true);
    $db->conn->Execute($sql);
  }

  function real_update($db, $cod, $fields) {
    $rs = $db->conn->Execute("select cod,titulo,tema,idioma,publicoalvo,descricao,resumo,comentarios,nivel_proposta, nivel_envolvimento from propostas where cod = $cod");
    $sql = $db->conn->GetUpdateSQL($rs, $fields, false, true);
    $db->conn->Execute($sql);
  }
